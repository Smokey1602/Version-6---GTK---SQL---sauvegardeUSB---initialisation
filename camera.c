...
{	
	...
	capture.set(CV_CAP_PROP_FRAME_HEIGHT,432);
...
{	
	...
	// NOTE : parametres du noir : (lowH=0 lowS=0 lowV=0, HighH=178 highS=255 highV=84 ! pour la detection d'abeille
		...
		{
		//////////      Detection des zones de couleurs    /////////////////////
			...
			//inRange(hsvcrop[i],Scalar(LowH,LowS,LowV,0),Scalar(HighH,HighS,HighV,0),masquecropViolet[i]);
		////////////////////////////////////////////////////////////////////////

		//////////      Application du filtre pour la suppression du bruit
			...
			//suppressbruit(masquecropViolet[i]);  
		////////////////////////////////////////////////////////////////////////
		
		//////////      Detection du point et determination des points de son vecteur et dessin de ce dernier
		...
		//////////      Comptage du passage des abeilles  //////////////////////    
	...
	/*
	imshow("cropedImage4 Bleu", masquecropBleu[3]);
	imshow("cropedImage4 Rouge", masquecropRouge[3]);
	imshow("cropedImage4 Vert", masquecropVert[3]);
	imshow("cropedImage4 Jaune", masquecropJaune[3]);
	imshow("cropedImage4 Total", masquecropTotal[3]);
	*/
	
	//on affiche ces images rognées (bebug)
	/*imshow("cropedImage1",  masquecropTotal[0]);
	imshow("cropedImage2",  masquecropTotal[1]);
	imshow("cropedImage3",  masquecropTotal[2]);
	imshow("cropedImage4",  masquecropTotal[3]);
	imshow("cropedImage5",  masquecropTotal[4]);
	imshow("cropedImage6",  masquecropTotal[5]);
	imshow("cropedImage7",  masquecropTotal[6]);
	imshow("cropedImage8",  masquecropTotal[7]);
	imshow("cropedImage9",  masquecropTotal[8]);
	imshow("cropedImage10", masquecropTotal[9]);*/
	
	
	/*
	//affiche le masque de l'image d'origine
	cvtColor(source,hsvsource,CV_BGR2HSV);
	inRange(hsvsource,Scalar(90,50,50,0),Scalar(130,255,255,0),masquesourcebleu);
	//suppressbruit(masquesourcebleu);
	imshow("Bleu",masquesourcebleu);

	cvtColor(source,hsvsource,CV_BGR2HSV);
	inRange(hsvsource,Scalar(0,50,50,0),Scalar(20,255,255,0),masquesourcerouge1);
	inRange(hsvsource,Scalar(140,50,50,0),Scalar(180,255,255,0),masquesourcerouge2);

	add(masquesourcerouge1,masquesourcerouge2,masquesourcerouge);
	suppressbruit(masquesourcerouge);
	imshow("Rouge",masquesourcerouge);
*/
